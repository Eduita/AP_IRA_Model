...
def create_full_correlation_table(data):
    correlation_table_full = (
        data.groupby(["time", "scenario"])
        .apply(
            lambda x: (
                x[
                    npv_columns
                    + list(sensitivity_sheet.columns.difference(["Time", "Scenario", "Simulation"]))
                ]
                .corr()
                .loc[npv_columns]
            )
...
final_csv_file_path = "sensitivity/correlation_{}.csv".format(
    "CBAM" if "CBAM" in file_directory else ""
)
