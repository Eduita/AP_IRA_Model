...
def extract_values(time, matching, results_df=wind_and_battery_excel):
    ...
    mask_high = (
        ...
        & (results_df["capex_description"] == "high")
        & (results_df["location"] == "high")
    )
    mask_low = (
        (results_df["time"] == time)
        & (results_df["matching"] == matching)
        & (results_df["capex_description"] == "low")
        & (results_df["location"] == "low")
    )
...
def calculate_battery_and_turbine_cost(technology, time, CAPEX_inputs):
    ...
    return (
        electricity_requirement_for_wind * CAPEX_inputs[f"Wind turbine CAPEX {time}"]
...
def calculate_final_CAPEX(
    technology, scenario, battery_and_turbine, electrode_cost, basic_equipment_costs, CAPEX_inputs
):
