class Carbon_Intensity_of_technology:
    def __init__(
        ...
        )  # (Tonne biomass/yr)*(Kg CO2e/Kg biomass)*(1000Kg biomass/tonne biomass) = kg Co2/yr
        self.biomass_intensity_kg_h2 = (
            biomass_carbon_emissions / hydrogen_production_kg_yearly
        )  # kg CO2/yr  /  kg h2/yr
    ...
    def electricity_emissions(self, T):
        if self.scenario == "A":
            ...
            raise ValueError(
                f"Invalid scenario: {self.scenario}. Scenario should be one of ['A', 'B', 'C', 'D']"
            )
