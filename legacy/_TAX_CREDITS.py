...
class TaxCreditCalculator:
    def __init__(
        ...
        self.carbon_intensity_calculator = Carbon_Intensity_of_technology(
            self.technology,
        ...
        self.AP_SMR_carbon_intensity_calculator = Carbon_Intensity_of_technology(
            "AP SMR",
            self.carbon_intensity,
            self.scenario,
            self.financial_inputs,
        ...
        self.electricity_demand_monthly = (
            self.carbon_intensity_calculator.electricity_requirements[self.technology][1]
            * 1000
            * 24
            * 30
        ...
        self.AP_SMR_carbon_intensity_calculator_start = (
            self.AP_SMR_carbon_intensity_calculator.total_emissions(self.start_month)
        )
        self.techology_emissions_start = self.carbon_intensity_calculator.total_emissions(
            self.start_month
        )
    ...
    def calculate_45V(self, month):
        if (
            ...
            or self.technology == "AP BH2S"
            or self.technology == "AP CCS"
        ):
            if self._is_operating(month) and self._is_within_lifetime(
                month, self.POLICY_INPUTS["45V lifetime"]
            ):
                carbon_intensity = self.carbon_intensity_calculator.total_emissions(
                    month - self.start_month
                )
                tier = self._get_45V_tier(carbon_intensity)
                return (
                    tier * self.hydrogen_production_monthly * 1000
                )  # Convert from $/kg H2 to $/tonne H2
            else:
                return 0
        else:
            return 0
    ...
    def calculate_45Q(self, month):
        if self.technology == "AP CCS":
            if self._is_operating(month) and self._is_within_lifetime(
                ...
                CO2_abated = (
                    max(0, AP_SMR_carbon_intensity - carbon_intensity)
                    * self.hydrogen_production_monthly
                )  # CO2 abated in tonne/month
                return (
                    self.POLICY_INPUTS["45Q"] * CO2_abated
                )  # Convert from $/tonne CO2 to $/kg CO2
            else:
                return 0
        else:
            return 0
    ...
    def calculate_48C(self, month):
        ...
        if (
            AP_SMR_carbon_intensity - 0.8 * AP_SMR_carbon_intensity
            > AP_SMR_carbon_intensity - techology_emissions
        ) and (AP_SMR_carbon_intensity - techology_emissions < 0):
            return 0
            ...
            if self.scenario != "C":
                ...
                if self.technology == "AP SMR":
                    ...
                    return self.electrode_cost * self.POLICY_INPUTS["48C"]  # Turned OFF
    ...
    def calculate_48E(self, month):
        ...
        if self.scenario == "C":
            if self.technology == "AP SMR":
                ...
                BATTERY_CAPACITY = (
                    self.CAPEX_inputs["battery_capacity"][self.technology] * 1000
                )  # kW
                SOLAR_CAPACITY = self.CAPEX_inputs["solar_capacity"][self.technology] * 1000  # kW

                # Calculations
                wind_capex = (
                    self.CAPEX_inputs[f"Wind turbine CAPEX {self.time}"] * WIND_CAPACITY
                )  # $/kW * kW = $
                battery_capex = (
                ...
                solar_capex = (
                    self.CAPEX_inputs[f"Solar PV CAPEX {self.time}"] * SOLAR_CAPACITY
                )  # $/kW * kW = $

                ELECTRICITY_CAPEX = wind_capex + battery_capex + solar_capex
                CREDITS_48E = self.POLICY_INPUTS["48E"]

                return ELECTRICITY_CAPEX * CREDITS_48E
        else:
            return 0
