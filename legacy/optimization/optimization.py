...
CAPEX_SPECS = {
    2023: {
        "high": {"wind": 1400, "battery": 1500, "solar": 1220},
        "low": {"wind": 1200, "battery": 800, "solar": 600},
    },
    2030: {
        "high": {"wind": 1200, "battery": 1200, "solar": 700},
        "low": {"wind": 750, "battery": 450, "solar": 250},
    },
}
...
def optimize_energy_system(
    ...
    w_t = [
        LpVariable(f"w_{t}", lowBound=0) for t in range(num_time_periods)
    ]  # wind generation profile
    c_t = [
        LpVariable(f"c_{t}", lowBound=0) for t in range(num_time_periods)
    ]  # battery charge profile
    d_t = [
        LpVariable(f"d_{t}", lowBound=0) for t in range(num_time_periods)
    ]  # battery discharge profile
    b_t = [LpVariable(f"b_{t}", lowBound=0) for t in range(num_time_periods)]  #
    S_t = [
        LpVariable(f"s_{t}", lowBound=0) for t in range(num_time_periods)
    ...
    curtailment_t = [
        CF_t_wind[t] * w.value() + CF_t_solar[t] * S.value() - optimal_w_t[t] - optimal_s_t[t]
        for t in range(num_time_periods)
    ]
...
def aggregate_to_monthly_post_processing(data, matching):
    if matching == "yearly":
        ...
        hours_per_month = [
            ...
            30 * 24,
            31 * 24,
        ]  # Adjust for leap YEAR_CASES if necessary
        start = 0
        for hours in hours_per_month:
            end = start + hours
                ...
                if index >= 0:  # Assuming you always want to start from the first row
                    ...
                    if os.path.isfile(file_path):
                        with open(file_path, "r") as f:
                            data = json.load(f)
                            capacity_factors[(year, town, state, type)] = [
                                list(data["electricity"].values())
                            ]
                            if town == "Pryor":
    ...
    with tqdm(total=total_iterations) as pbar:
        for year in YEAR_CASES:
            for matching in MATCHING_CASES:
                for state, town in zip(
                    list(us_ap_plants_df["State"]), list(us_ap_plants_df["City"])
                ):
                    ...
                    for capex_type in CAPEX_CASES:
                        for technology in TECHNOLOGY_CASES:
                            ...
                            COST_battery = (
                                CAPEX_SPECS[year][capex_type]["battery"] * 1000 / 4
                            )  # $/MW
                            ...
                            curtailment_monthly = aggregate_to_monthly_post_processing(
                            ...
                            total_gen_monthly = aggregate_to_monthly_post_processing(
                                total_gen, matching
                            )
