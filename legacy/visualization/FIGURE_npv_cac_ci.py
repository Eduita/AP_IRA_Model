...
for key, file_data in datafiles.items():
    logging.info(f"Processing file data for key: {key}")
    for sheet_name, df in file_data.items():
        ...
        if sheet_name.startswith("CI_"):
            ...
            logging.debug(
                f"Grouping data by time and scenario for sheet: {sheet_name}"
            )  # Log data grouping
            ...
            for (time, scenario), group_df in grouped:
                ...
                logging.debug(
                    f"Grouped data added for {sheet_name}, time: {time}, scenario: {scenario}"
                )  # Log addition of grouped data

for time_key, matching_data in cleaned_datafiles_with_matching.items():
    logging.info(
        f"Processing matching data for time: {time_key}"
    )  # Log processing for each time period
    for matching_type, scenario_data in matching_data.items():
        logging.debug(
            f"Found CI data in matching type: {matching_type} for time: {time_key}"
        )  # Log when CI data is found
        if "CI" in scenario_data:
            ...
            logging.debug(
                f"Removed original ungrouped CI data from matching type: {matching_type} for time: {time_key}"
            )  # Log removal of original CI data
...
def plot_sheet_subplot(
    ...
    for index, tech in enumerate(technologies):
        ...
        ax.fill_between(
            x_values, min_values.values, max_values.values, color=colors[index], alpha=alpha
        )
    ...
    if annotate:
        ...
        ax.annotate(
            ...
            arrowprops=dict(
                facecolor="black", arrowstyle="<->", linestyle="dashed", alpha=0.5
            ),  # Arrow properties
            ha="center",
            va="center",
        )
    ...
    if left:
        ax.set_ylabel(
            r"Carbon Intensity [$\frac{kgCO_2 \ eq}{Kg \ H_2}$]",
            fontweight="bold",
            fontsize=14,
            labelpad=1,
        )
...
fig, axs = plt.subplots(
    ...
    2,
    figsize=(10, 5),
    gridspec_kw={"width_ratios": [1.1, 1.1], "height_ratios": [1], "wspace": 0.22, "hspace": 0.15},
)
...
plt.savefig(rf"{ROOT_FILES}/visuals/CI_CBAM v2.png", dpi=500)
plt.close()


# Plotting NPVs
def compare_policy_scenarios_subplot(
    time, matching, scenario, colors, ax, title, show_legend=True, bottom=True, left=True, top=False
):
    """
    ...
    if matching == "monthly" or matching == "CBAM":
        ranges = {"A": [-450, 250], "B": [-450, 250], "C": [-450, 250], "D": [-450, 250]}
        ax.set_ylim(*ranges[scenario])
    elif matching in ["hourly", "yearly"]:
        ranges = {"A": [-200, 400], "B": [-200, 400], "C": [-2000, 400], "D": [-600, 400]}
        ax.set_ylim(*ranges[scenario])
    ...
    updated_labels = [
        label.replace("_", " ").replace("AP", "").replace("NPV", "") for label in column_names
    ]
    if not bottom:
    ...
    if left:
        ax.set_ylabel(
            r"NPV [$\frac{2023\$}{Tonne \ NH_3}$], " + str(time + 3), fontweight="bold", fontsize=14
        )
...
compare_policy_scenarios_subplot(
    2023,
    "monthly",
    "B",
    colors,
...
plt.savefig(rf"{ROOT_FILES}/visuals/NPV Baseline v2.png", dpi=300)
plt.close()
...
plt.savefig(rf"{ROOT_FILES}/visuals/NPV Matching v2.png", dpi=300)
plt.close()
...
plt.savefig(rf"{ROOT_FILES}/visuals/NPV CBAM v2.png", dpi=300)
plt.close()
...
# Plotting CACs
def compare_cac_scenarios_subplot(
    time, matching, scenario, colors, ax, title, show_legend=True, bottom=True, left=True
):
    """
    ...
    ax.fill_betweenx(ax.get_ylim(), 18, 20, color="gray", alpha=0.4)
    if matching == "hourly" and scenario == "C":
        ax.text(
            15,
            midpoint,
    ...
    ax.fill_betweenx(ax.get_ylim(), 190, 230, color="gray", alpha=0.4)

    ax.axhline(y=0.0, color="black", linestyle="--", linewidth=1.5, alpha=0.3)
    ...
    ax.fill_betweenx(ax.get_ylim(), 30, 89, color="gray", alpha=0.4)
    ...
    if bottom:
        ax.set_xlabel(
            r"CAC [$\mathbf{\frac{2023\$}{Tonne \ CO_2}}$]", fontweight="bold", fontsize=11
        )
    ...
    if left:
        ax.set_ylabel(
            r"NPV [$\mathbf{\frac{2023\$}{Tonne \ NH_3}}$], " + str(time + 3),
            fontweight="bold",
            fontsize=14,
        )
...
compare_cac_scenarios_subplot(
    2030, "monthly", "B", colors, axs[1, 0], "", show_legend=False, bottom=True, left=True
)
compare_cac_scenarios_subplot(
    2030, "monthly", "C", colors, axs[1, 1], "", show_legend=False, bottom=True, left=False
)
compare_cac_scenarios_subplot(
    2030, "monthly", "D", colors, axs[1, 2], "", show_legend=False, bottom=True, left=False
)

plt.savefig(rf"{ROOT_FILES}/visuals/CAC Baseline v2.png", dpi=500)
plt.close()
...
compare_cac_scenarios_subplot(
    2030, "hourly", "C", colors, axs[1, 0], "", show_legend=False, bottom=True, left=True
)
compare_cac_scenarios_subplot(
    2030, "yearly", "C", colors, axs[1, 1], "", show_legend=False, bottom=True, left=False
)
compare_cac_scenarios_subplot(
    2030, "hourly", "D", colors, axs[1, 2], "", show_legend=False, bottom=True, left=False
)
compare_cac_scenarios_subplot(
    2030, "yearly", "D", colors, axs[1, 3], "", show_legend=False, bottom=True, left=False
)

plt.savefig(rf"{ROOT_FILES}/visuals/CAC Matching v2.png", dpi=500)
plt.close()
