...
data = pd.read_excel(
    r"C:\Users\eduar\OneDrive\Desktop\PycharmProjects\Ammonia Project\pythonProject1\NOVEMBER AP Model\power_markets\state_level_datasets\AP_location_dataset.xlsx",
    sheet_name="US AP Plants",
)
...
def API_call(params, time, state, town, url=url):
    ...
    metadata = parsed_response["metadata"]
    data_cf.to_json(
        f"Wind data/more capacity data/data/{time}_{town}_{state}_wind_capacity_data.json"
    )
