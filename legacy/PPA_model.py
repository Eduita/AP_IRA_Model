...
class Stochastic_DCF:
    ...
    def calcOPEX(self, T):
        ...
        BATTERY_CAPACITY = self.CAPEX_inputs['battery_capacity']['AP AEC'][self.LOCATION] * 1000 / 4  # kWh
...
def run_simulation():
    ...
    LCOE_i = 0
    n=0
    LCOE_data = pd.DataFrame(columns=['time', 'matching', 'cost', 'policy', 'LCOE', 'low_town', 'high_town'])
            ...
            for policy in policies:
                for CF_type in CF_CASES:
                    ...
                    CAPEX_inputs['wind_capacity'] = {
                        'AP AEC': {
                            'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['wind_capacity'],
                            'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['wind_capacity']
                        }
                    }
                    CAPEX_inputs['battery_capacity'] = {
                        'AP AEC': {
                            'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['battery_capacity'],
                            'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['battery_capacity']
                        }
                    }
                    CAPEX_inputs['solar_capacity'] = {
                        'AP AEC': {
                            'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['solar capacity'],
                            'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['solar capacity']
                        }
                    }
                    ...
                    ELEC_INPUTS['curtailment'] = { #MWh/month
                            'AP AEC':{
                                'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['curtailment'],
                                'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['curtailment']
                            },
                        }
                    ELEC_INPUTS['total_generation']= {
                            'AP AEC':{
                                'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['total_gen'],
                                'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['total_gen']
                            }
                        }
                    ELEC_INPUTS['discharge']= {  # MWh/month
                            'AP AEC': {
                                'high': get_closest_quantile_data(time, matching, 'AP AEC high')[0]['demand'],
                                'low': get_closest_quantile_data(time, matching, 'AP AEC high')[1]['demand']
                            }
                        }
                    ...
                    final_CAPEX = CAPEX(CAPEX_inputs).capex_components(CAPEX_electricity(time, CAPEX_inputs, CF_type))

                    NPV_object = Stochastic_DCF(start, L, policy, 'D', FIN_INPUTS, final_CAPEX, CAPEX_inputs
                                                , POLICY_INPUTS, ELEC_INPUTS, MIOPEX_INPUTS
                                                , battery_and_turbine_data_final, CF_type, LCOE_i, matching=matching)
                    def NPV(LCOE):
                        return NPV_object.calculate_NPV(LCOE)
                    result = root_scalar(NPV, bracket=[0,2000])
                    metric = [time, matching, CF_type, policy, result.root,
