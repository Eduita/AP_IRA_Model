...
for key, file_data in datafiles.items():
    ...
    for sheet_name, df in file_data.items():
        ...
        if sheet_name.startswith("CI_"):
            ...
            logging.debug(f'Grouping data by time and scenario for sheet: {sheet_name}')  # Log data grouping
            ...
            for (time, scenario), group_df in grouped:
                ...
                logging.debug(f'Grouped data added for {sheet_name}, time: {time}, scenario: {scenario}')  # Log addition of grouped data

for time_key, matching_data in cleaned_datafiles_with_matching.items():
    logging.info(f'Processing matching data for time: {time_key}')  # Log processing for each time period
    for matching_type, scenario_data in matching_data.items():
        logging.debug(f'Found CI data in matching type: {matching_type} for time: {time_key}')  # Log when CI data is found
        if "CI" in scenario_data:
            ...
            logging.debug(f'Removed original ungrouped CI data from matching type: {matching_type} for time: {time_key}')  # Log removal of original CI data
