    ...
    for start in starts:
        ...
        for scenario in scenarios:
            for sim in range(simulations):
                ...
                Market_inputs["NH3_initial_price"] = (
                    Market_inputs["NG_initial_price"] * Market_inputs["NH3 markup"]
                )  # Completely correlate the markets
                ...
                for tech in technologies[1:]:
                    if tech != "AP AEC":
                        CAPEX_inputs["wind_capacity"][tech] = get_closest_quantile_data(
                            time, matching, tech
                        )[1]["wind_capacity"]
                        CAPEX_inputs["battery_capacity"][tech] = get_closest_quantile_data(
                            time, matching, tech
                        )[1]["battery_capacity"]
                        CAPEX_inputs["solar_capacity"][tech] = get_closest_quantile_data(
                            time, matching, tech
                        )[1]["solar capacity"]
                    else:
                        CAPEX_inputs["wind_capacity"][tech] = (
                            get_closest_quantile_data(time, matching, tech + " low")[1][
                                "wind_capacity"
                            ],
                        )
                        CAPEX_inputs["battery_capacity"][tech] = get_closest_quantile_data(
                            time, matching, tech + " low"
                        )[1]["battery_capacity"]
                        CAPEX_inputs["solar_capacity"][tech] = get_closest_quantile_data(
                            time, matching, tech + " low"
                        )[1]["solar capacity"]
                ...
                AP_AEC_curtailment_list = [
                    low + AP_AEC_curtailment_correlative_prob * (high - low)
                    for low, high in zip(
                        wind_and_battery_data[time][matching]["AP AEC low"]["curtailment"],
                        wind_and_battery_data[time][matching]["AP AEC high"]["curtailment"],
                    )
                ]

                AP_AEC_discharge_list = [
                    low + AP_AEC_curtailment_correlative_prob * (high - low)
                    for low, high in zip(
                        wind_and_battery_data[time][matching]["AP AEC low"]["discharge"],
                ...
                carbon_intensity["stack"]["AP SMR"] = np.mean(
                    [AP_SMR_lower_bound, AP_SMR_upper_bound]
                )
                ...
                carbon_intensity["natural gas"] = (np.mean([0.01, 7.9]) / 1000) * carbon_intensity[
                ...
                def calculate_battery_and_turbine_cost(technology, time, CAPEX_inputs):
                    ...
                    electricity_requirement_for_wind = (
                        CAPEX_inputs["wind_capacity"][technology] * 1000
                    )  # kW
                    electricity_requirement_for_battery = (
                        CAPEX_inputs["battery_capacity"][technology] * 1000
                    )  # kW

                    return (
                        electricity_requirement_for_wind
                        * CAPEX_inputs[f"Wind turbine CAPEX {time}"]
                        + electricity_requirement_for_battery
                        * CAPEX_inputs[f"Battery Storage CAPEX {time}"]
                ...
                if CAC:
                    CAC_metrics = [
                        ...
                        Stochastic_DCF(
                            ...
                            matching=matching,
                            deterministic=True,
                        ).carbon_abatement_cost(value=1, set_value=True),
                        Stochastic_DCF(
                            "AP BH2S",
                            start,
                            L,
                ...
                if Potential_TC:
                    Potential_metrics = [
                        ...
                        Stochastic_DCF(
                            ...
                            matching=matching,
                            deterministic=True,
                        ).carbon_abatement_cost(value=1, set_value=True, absolute=True)
                        / 1000000000,
                        Stochastic_DCF(
                            "AP BH2S",
                            start,
                        ...
                        Stochastic_DCF(
                            ...
                            matching=matching,
                            deterministic=True,
                        ).carbon_abatement_cost(value=IRA_credits["TCvalue"], absolute=True)
                        / 1000000000,
                        Stochastic_DCF(
                            "AP BH2S",
                            start,
                ...
                if absolute_support:
                    AP_CCS = Stochastic_DCF(
                        ...
                        matching=matching,
                        deterministic=True,
                    ).carbon_abatement_cost(1, set_value=True, separate=True, absolute=True)
                    ...
                    absolute_support_metric = [
                        ...
                        sim,
                        AP_CCS[0] / 1000000000,
                        AP_BH2S[0] / 1000000000,
                        AP_AEC[0] / 1000000000,
                        AP_CCS[1] / 1000000000,
                        AP_BH2S[1] / 1000000000,
                        AP_AEC[1] / 1000000000,
                        AP_CCS[2] / 1000000000,
                        AP_BH2S[2] / 1000000000,
                        AP_AEC[2] / 1000000000,
                        AP_CCS[3] / 1000000000,
                        AP_BH2S[3] / 1000000000,
                        AP_AEC[3] / 1000000000,
                        AP_CCS[4] / 1000000000,
                        AP_BH2S[4] / 1000000000,
                        AP_AEC[4] / 1000000000,
                    ]
...
NPV_dataset = run_simulation(
    1 * mult, NPV=True, isCBAM=CBAM, matching=matching
)  # checked all scenarios v5
print(NPV_dataset)
