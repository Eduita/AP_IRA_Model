...
def process_capex_data(file_path, sheet_name, output_path):
    ...
    grouped_data = (
        capex_component_data.groupby(["time", "scenario"]).apply(apply_mean).reset_index()
    )
    ...
    rows_order = [
        ...
        "piping",
        "electrical",
        "building_process_auxiliary",
        "service_facilities_and_yard_improvements",
        "land",
        "engineering_supervision",
        "legal_expenses",
        "construction_expense_and_contractors_fee",
        "contingency",
        "FCI",
        "WC",
        "CAPEX",
    ...
    def create_table(group):
        table_data = {
            col_name: [
                group[col][key if key not in ["FCI", "WC", "UC"] else key] for key in rows_order
            ]
            for col, col_name in columns_mapping.items()
        }
...
def process_roi_data(file_path, sheet_name, output_path):
    ...
    mean_roi_data = (
        roi_data.groupby(["time", "scenario"]).mean().reset_index().drop(columns=["simulation"])
    )
...
capex_columns = [
    ...
    "AP_BH2S_CAPEX",
    "AP_AEC_CAPEX",
]  # Replace with your CAPEX columns
