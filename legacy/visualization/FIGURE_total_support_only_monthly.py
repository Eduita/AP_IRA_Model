...
def adapted_box_plot_on_ax(
    ...
    if colors is None:
        colors = {
            "Cash-Equivalent (9.3%)": "#b8f4ff",
            "Potential (2%)": "#5c5c5c",
            "Cash-Equivalent (2%)": "#8aa5aa",
        }
    ...
    for ce_col, soc_pol, pot_col in zip(cash_eq_cols, cash_eq__social_cols, potential_cols):
        ...
        box_plot_data.append(
            pd.DataFrame(
                {
                    "Value": data[ce_col],
                    "Group": [label] * len(data),
                    "Type": ["Cash-Equivalent (9.3%)"] * len(data),
                }
            )
        )
        box_plot_data.append(
            pd.DataFrame(
                {
                    "Value": data[soc_pol],
                    "Group": [label] * len(data),
                    "Type": ["Cash-Equivalent (2%)"] * len(data),
                }
            )
        )
        box_plot_data.append(
    ...
    # Create the box plot
    sns.boxplot(
        x="Group", y="Value", hue="Type", data=box_plot_data, ax=ax, palette=colors, width=0.6
    )
    ax.set_xlabel("")
    ax.set_title(title if top else "", fontweight="bold", fontsize=20)
    ax.set_xticklabels(
        [label.get_text().replace("_", " ") for label in ax.get_xticklabels()] if bottom else []
    )
    ax.set_ylabel(
        r"Tax Credits [$\frac{2023\$}{Kg \ H_2} $]" + f",{time + 3}" if left else "",
        fontsize=16,
        fontweight="bold",
    )
...
adapted_box_plot_on_ax(
    cleaned_datafiles_with_matching,
    time,
    "B",
    matching,
    ax=axs[0, 0],
