    ...
    for start in starts:
        ...
        for scenario in scenarios:
            for sim in range(simulations):
                ...
                Market_inputs['NH3_initial_price'] = Market_inputs['NG_initial_price']*Market_inputs["NH3 markup"] #Completely correlate the markets
                ...
                for tech in technologies[1:]:
                    if tech != 'AP AEC':
                        CAPEX_inputs['wind_capacity'][tech] = get_closest_quantile_data(time, matching, tech)[1]['wind_capacity']
                        CAPEX_inputs['battery_capacity'][tech] = get_closest_quantile_data(time, matching, tech)[1]['battery_capacity']
                        CAPEX_inputs['solar_capacity'][tech] = get_closest_quantile_data(time, matching, tech)[1]['solar capacity']
                    else:
                        CAPEX_inputs['wind_capacity'][tech] = get_closest_quantile_data(time, matching, tech + ' low')[1]['wind_capacity'],
                        CAPEX_inputs['battery_capacity'][tech] = get_closest_quantile_data(time, matching, tech + ' low')[1]['battery_capacity']
                        CAPEX_inputs['solar_capacity'][tech] = get_closest_quantile_data(time, matching, tech + ' low')[1]['solar capacity']
