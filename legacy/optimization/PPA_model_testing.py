...
def run_simulation():
    ...
    LCOE_data = pd.DataFrame(
        columns=["time", "matching", "cost_type", "town", "state", "policy", "LCOE", "CF sum"]
    )
    ...
    with tqdm(total=TOTAL_ITERATIONS) as pbar:
        for policy in policies:
            for index, location in LOCATION_DATA.iterrows():
                ...
                CAPEX_inputs["wind_capacity"] = {"AP AEC": location["wind_capacity"]}
                CAPEX_inputs["battery_capacity"] = {"AP AEC": location["battery_capacity"]}
                CAPEX_inputs["solar_capacity"] = {"AP AEC": location["solar capacity"]}
                ...
                ELEC_INPUTS["total_generation"] = {"AP AEC": location["total_gen"]}
                ...
                final_CAPEX = CAPEX(CAPEX_inputs).capex_components(
                    CAPEX_electricity(time, CAPEX_inputs, location["capex_type"])
                )
