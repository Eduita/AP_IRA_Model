...
def run_simulation(
    ...
    CAC_data = pd.DataFrame(
        columns=["time", "scenario", "simulation", "AP_CCS_CAC", "AP_BH2S_CAC", "AP_AEC_CAC"]
    )
    ...
    CAPEX_component_data = pd.DataFrame(
        columns=["time", "scenario", "sim", "AP_SMR_NPV", "AP_CCS_NPV", "AP_BH2S_NPV", "AP_AEC_NPV"]
    )
    ...
    location_info_df = pd.DataFrame(columns=["time", "scenario", "simulation", "town", "state"])
    ...
    for start in starts:
        ...
        for scenario in scenarios:
            for sim in range(simulations):
                ...
                INPUT_PARAMETERS_PATH = det_INPUT_PARAMETERS_PATH
                ...
                INSTANTIATED_MODEL_INPUTS = instantiated_input.average_values_from_JSON_inputs(
                ...
                def AEC_rand_dist(
                    ...
                    if isinstance(low_item, float) and isinstance(high_item, float):
                        ...
                        return [
                            low_item[i] + (high_item[i] - low_item[i]) * rand_num
                            for i in range(len(low_item))
                        ]
                ...
                for tech in technologies[1:]:
                    if tech != "AP AEC":
                        ...
                        CAPEX_inputs["battery_capacity"][tech] = OPTIMIZATION_ROW[
                            "battery_capacity"
                        ]
                        ...
                        CAPEX_inputs["wind_capacity"][tech] = AEC_rand_dist(
                            OPTIMIZATION_ROW_LOW["wind_capacity"],
                            OPTIMIZATION_ROW_HIGH["wind_capacity"],
                        )
                        CAPEX_inputs["battery_capacity"][tech] = AEC_rand_dist(
                            OPTIMIZATION_ROW_LOW["battery_capacity"],
                            OPTIMIZATION_ROW_HIGH["battery_capacity"],
                        )
                        CAPEX_inputs["solar_capacity"][tech] = AEC_rand_dist(
                            OPTIMIZATION_ROW_LOW["solar capacity"],
                            OPTIMIZATION_ROW_HIGH["solar capacity"],
                        )
                ...
                financial_inputs["operating_hours_per_year"] = (
                    365 * 24 * financial_inputs["availability"]
                )
                ...
                Market_inputs["PPA_pricing_for_C"] = {
                    time: {
                        policy: Market_inputs["PPA_pricing"][time]["yearly"][policy]
                        for policy in policies
                    }
                    for time in times
                }
                ...
                IRA_credits["45V"] = {
                    float(key): value for key, value in IRA_credits["45V"].items()
                }
                ...
                def calculate_battery_and_turbine_cost(technology, time, CAPEX_inputs):
                    ...
                    # Calculations
                    wind_capex = (
                        CAPEX_inputs[f"Wind turbine CAPEX {time}"] * WIND_CAPACITY
                    )  # $/kW * kW = $
                    battery_capex = (
                    ...
                    solar_capex = (
                        CAPEX_inputs[f"Solar PV CAPEX {time}"] * SOLAR_CAPACITY
                    )  # $/kW * kW = $
                ...
                def calculate_electrode_cost(time, electricity_requirements, CAPEX_inputs):
                    ...
                    return (
                        electricity_requirements["AP AEC"][1] * CAPEX_inputs[stack_cost_key] * 1000
                    )
                ...
                def calculate_final_CAPEX(
                    ...
                    if scenario == "C" and technology != "AP SMR":
                        ...
                        if technology == "AP AEC":
                            final_cost = capex_obj2.add_cost_outside_of_equipment_list(
                                electrode_cost
                            )
                        else:
                ...
                for technology in technologies:
                    ...
                    battery_and_turbine = calculate_battery_and_turbine_cost(
                        technology, time, CAPEX_inputs
                    )
                ...
                for tecnology in technologies:
                    ...
                    if scenario == "C" and tecnology != "AP SMR":
                        electricity_requirement_for_wind = (
                            CAPEX_inputs["wind_capacity"][tecnology] * 1000
                        )  # kW
                ...
                if NPV:
                    ...
                    message = (
                        "Calculating NPV..." + str(time) + " " + str(scenario) + " " + str(sim)
                    )
                ...
                if NP_NPV:
                    ...
                    message = (
                        "Calculating NP NPV..." + str(time) + " " + str(scenario) + " " + str(sim)
                    )
                    sys.stdout.write("\r" + message)
                    sys.stdout.flush()
                ...
                if CAC:
                    ...
                    message = (
                        "Calculating CAC..." + str(time) + " " + str(scenario) + " " + str(sim)
                    )
                    sys.stdout.write("\r" + message)
                    sys.stdout.flush()
                ...
                if CI:
                    ...
                    for months in multiples_of_12_CI:
                        scenario = scenario
                        CI_time_SMR_vals = Carbon_Intensity_of_technology(
                            "AP SMR",
                            carbon_intensity,
                            scenario,
                            financial_inputs,
                            ...
                            aeo22_data,
                            aeo23_data,
                        ).total_emissions(months)
                        CI_time_CCS_vals = Carbon_Intensity_of_technology(
                            "AP CCS",
                            carbon_intensity,
                            scenario,
                            financial_inputs,
                        ...
                        CI_time_BH2S_vals = Carbon_Intensity_of_technology(
                            "AP BH2S",
                            carbon_intensity,
                            scenario,
                            financial_inputs,
                        ...
                        CI_time_AEC_vals = Carbon_Intensity_of_technology(
                            "AP AEC",
                            carbon_intensity,
                            scenario,
                            financial_inputs,
                ...
                if CAPEX_OPEX:
                    ...
                    for T in range(601):
                        Point_SMR = (
                            ...
                            / (
                                engineering_inputs["NH3"]
                                * 365
                                / 12
                                * financial_inputs["availability"]
                            ),
                        )
                        Point_CCS = (
                            Stochastic_DCF(
                ...
                if absolute_support:
                    AP_CCS = Stochastic_DCF(
                        ...
                        isCBAM=isCBAM,
                        matching=matching,
                    ).separate_total_support()
                    AP_BH2S = Stochastic_DCF(
                        "AP BH2S",
                        start,
                        L,
                        sim,
                    ...
                    absolute_support_metric = [
                        ...
                        AP_CCS["45Y"],
                        AP_BH2S["45Y"],
                        AP_AEC["45Y"],  # This is a placeholder for 48C. Ignore.
                        AP_CCS["48E"],
                        AP_BH2S["48E"],
                        AP_AEC["48E"],
                    ]
                ...
                if ROI:
                    ...
                    message = (
                        "Calculating ROI..." + str(time) + " " + str(scenario) + " " + str(sim)
                    )
                    sys.stdout.write("\r" + message)
                    sys.stdout.flush()
                ...
                if El_market and (scenario == "A" or scenario == "B"):
                    ...
                    message = (
                        "Calculating Electricity Market..."
                        + str(time)
                        + " "
                        + str(scenario)
                        + " "
                ...
                if Sensitivity:
                    A_elec_cost = np.mean(
                        brownian_motion(
                            drift=Market_inputs["El_drift"][1],
                            std_dev=Market_inputs["El_STD"],
                            n_steps=601,
                            seed=sim,
                        ).uncorrelated_GBM(Market_inputs["El_initial_price"])
                    )

                    B_elec_cost = np.mean(
                        brownian_motion(
                            drift=Market_inputs["El_drift"][0],
                            std_dev=Market_inputs["El_STD"],
                            n_steps=601,
                            seed=sim,
                        ).uncorrelated_GBM(Market_inputs["El_initial_price"])
                    ...
                    NH3_average, NG_average = brownian_motion(
                        drift=Market_inputs["NG_drift"],
                        std_dev=Market_inputs["NG_std"],
                        correlation=0.8,
                        n_steps=601,
                        seed=sim,
                    ).correlated_GBM(
                        [Market_inputs["NH3_initial_price"], Market_inputs["NG_initial_price"]]
                    )
                    ...
                    IRA_Market_value = np.mean(
                        [IRA_credits["TCvalue"][i] for i in IRA_credits["TCvalue"].keys()]
                    )
                    PPA_pricing = (
                        Market_inputs["PPA_pricing"][time][matching][True] if scenario == "D" else 0
                    )
                    Curtailment = (
                        Market_inputs["PPA_pricing_for_C"][time][True] if scenario == "C" else 0
                    )
                    Battery_CAPEX = (
                        CAPEX_inputs[f"Battery Storage CAPEX {time}"] if scenario == "C" else 0
                    )
                    Wind_CAPEX = (
                        CAPEX_inputs[f"Wind turbine CAPEX {time}"] if scenario == "C" else 0
                    )
                    ...
                    for tech in technologies:
                        metric_Sense = [
                            ...
                            scenario,
                            sim,
                            CAPEX_sensitivity[tech],
                            electricityCost,
                            MI_OPEX_sensitivity[tech],
                            Feedstock_price[tech],
                            NH3_average,
                            CI_sens[tech],
                            Battery_var_OPEX,
                            Battery_OPEX,
                ...
                if quality_assurance:
                    ...
                    dataframes_to_excel(
                        dfs, f"quality_assurance/quality_{matching}_{time}_{scenario}_{sim}.xlsx"
                    )
                ...
                if location_info:
                    metric = [
                        ...
                        scenario,
                        sim,
                        get_capacity_data(time, matching, "AP CCS")["town"],
                        get_capacity_data(time, matching, "AP CCS")["state"],
                    ]
                    location_info_df.loc[len(location_info_df)] = metric
...
matching = "hourly"  # options: hourly, monthly, and yearly
...
# #Simulations to do:
# Location_info = run_simulation(100*mult, location_info=True, isCBAM=CBAM, matching=matching)
NPV_dataset = run_simulation(1, NPV=True, isCBAM=CBAM, matching=matching)

print(NPV_dataset)
# NP_NPV_dataset = run_simulation(100*mult, NP_NPV=True, isCBAM=CBAM, matching=matching)
# CAC_dataset = run_simulation(100*mult, CAC=True, isCBAM=CBAM, matching=matching)
# CI_dataset = run_simulation(100*mult, CI=True, isCBAM=CBAM, matching=matching)
# TC_dataset = run_simulation(100*mult, Potential_TC=True, isCBAM=CBAM, matching=matching)
# CAPEX_OPEX_dataset = run_simulation(100*mult, CAPEX_OPEX=True, isCBAM=CBAM, matching=matching)
# absolute_support_dataset = run_simulation(100*mult, absolute_support=True, isCBAM=CBAM, matching=matching)
# ROI_dataset = run_simulation(100*mult, ROI=True, isCBAM=CBAM, matching=matching)
# El_dataset = run_simulation(100*mult, El_market=True, isCBAM=CBAM, matching=matching)
#
dfs = {
    "NPV": NPV_dataset,
...
dataframes_to_excel(dfs, f"v14_DATASET{'_CBAM' if CBAM else ''}_{matching}_500.xlsx")
